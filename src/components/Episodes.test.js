...
test("Episodes shows data when rerendered with new episodes data", async () => {
  const { queryAllByTestId } = render(<Episodes episodes={[]} />);
  //   assert that there are no episodes listed when the component first renderes
  expect(queryAllByTestId(/episode-data/i)).toHaveLength(0);
const wrapper = render(<Episodes episodes={mockEpisodes} />);
const episodes = await wrapper.queryByTestId(/episode-data/i);
